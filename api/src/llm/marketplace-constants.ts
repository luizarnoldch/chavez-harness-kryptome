// keep-in-sync with cli/src/llm/marketplace-constants.ts
...
export function validateMarketplaceInstallBody(
  body: unknown,
):
  | { ok: true; kind: "skill"; id: string }
  | { ok: false; error: string } {
  const kind = String((body as { kind?: string }).kind || "");
  const id = String((body as { id?: string }).id || "").trim();
  if (kind === "mcp") return { ok: false, error: MARKETPLACE_KIND_LAYER };
  if (kind !== "skill") return { ok: false, error: MARKETPLACE_KIND_LAYER };
  return { ok: true, kind: "skill", id };
}
